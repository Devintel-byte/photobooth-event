...
export const ResultPage = () => {
  ...
  const createCompositeImage = async (genUrl: string) => {
    ...
    try {
      ...
      setCompositeImage(genUrl); // fallback to raw image
    }
  };
  ...
  // upload composite in background once ready
  useEffect(() => {
    if (compositeImage) {
      uploadToEdgestore(compositeImage).then((url) => {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 p-4 md:p-8">
      ...
      <div className="max-w-md mx-auto bg-slate-100 rounded-xl shadow-md overflow-hidden p-6 md:p-8">
        <div className="text-center">
          {result.status === 'COMPLETED' && compositeImage && (
